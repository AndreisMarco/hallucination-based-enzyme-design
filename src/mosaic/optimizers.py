    ...
    for _iter in range(steps):
        ...
        for i in range(50):
            ...
            if np.all(proposal == sequence):
                ...
                # _print_iter(_iter, {"": aux_0, "time": time.time() - start_time}, v_0)
                # continue
            else:
                break
...
def batched_simplex_APGM(
    *,
    ...
    if log_trajectory:
        loggers = [TrajectoryLogger() for _ in range(B)]
    ...
    for _iter in range(n_steps):
        ...
        for i in range(B):
            ...
            aux_i.update({"optim": {
                ...
                "nnz": average_nnz,
                "time": time.time() - start_time,
                "pssm": clean_pssm(x[i], loss_function) if not logspace \
                        else clean_pssm(jax.nn.softmax(x[i]), loss_function),
                }})
...
def batch_greedy_descent(
    ...
    *,
    ...
    for _iter in range(steps):
        ...
        if picked is None:
            print(f"step {_iter}: neighbourhood exhausted, stopping")
            break
        cands, _ = picked
        m = cands.shape[0]
    ...
    if not log_trajectory:
        ...
        logger.clean_trajectory()
        return best_seq, best_val, logger
