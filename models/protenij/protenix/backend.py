...
class softmax(eqx.Module):
    ...
    def __call__(self, x):
        return jax.nn.softmax(x.astype(jnp.float32), axis=self.dim).astype(x.dtype)
...
def _convert_bf16(model):
    """Convert model weights to bf16, excluding only layernorms (kept in fp32 for stability)."""
    def convert_leaf(leaf):
        if eqx.is_array(leaf) and leaf.dtype == jnp.float32:
            return leaf.astype(jnp.bfloat16)
        return leaf

    new_fields = {}
    for field in model.__class__.__dataclass_fields__:
        if hasattr(model, field):
            value = getattr(model, field)
            if "layernorm" in field.lower():
                new_fields[field] = value  # Keep layernorms in fp32
            else:
                # Convert this field to bf16
                new_fields[field] = jax.tree.map(
                    convert_leaf,
                    value,
                    is_leaf=eqx.is_array,
                )
...
def load_model(name_or_path: str, bf16: bool = False):
    ...
    Args:
        ...
        bf16: If True, convert model weights to bf16 (except diffusion head,
            confidence head, and layernorms which stay in fp32).
    """
    ...
    model = eqx.tree_deserialise_leaves(f"{path}.eqx", skeleton)

    if bf16:
        print("Converting model to bf16")
        model = _convert_bf16(model)
