...
class TriangleMultiplication(AbstractFromTorch):
    ...
    def __call__(
        ...
        dtype = self.linear_a_p.weight.dtype
        if mask is None:
...
class Attention(AbstractFromTorch):
    ...
    def __call__(
        ...
        q = q / jnp.sqrt(self.c_hidden).astype(q.dtype)
...
class TriangleAttention(AbstractFromTorch):
    ...
    def __call__(
        ...
        # [*, I, 1, 1, J]
        mask_bias = (self.inf * (mask - 1))[..., :, None, None, :].astype(dtype)
...
class OuterProductMean(AbstractFromTorch):
    ...
    def __call__(
        ...
        # if mask is None:
        mask = jnp.ones(m.shape[:-1], dtype=m.dtype)  # , dtype=bool)
        ...
        norm = (norm + self.eps).astype(outer.dtype)
...
def average_over_atoms(
    ...
    def _helper(x_atom):
        ...
        atoms_per_residue_count = jax.lax.scan(
            body_function,
            init=jnp.zeros((n_res,), dtype=x_atom.dtype),
            xs=(atom_to_token_idx, jnp.ones(n_atoms, dtype=x_atom.dtype)),
        )[0]

        accumulated_sum = jax.lax.scan(
            body_function, init=jnp.zeros((n_res, d), dtype=x_atom.dtype), xs=(atom_to_token_idx, x_atom)
        )[0]
...
class AtomAttentionEncoder(eqx.Module):
    ...
    def __call__(self, input_feature_dict: dict[str], r_l=None, s=None, z=None):
        ...
        dtype = self.linear_no_bias_ref_pos.weight.dtype
        ...
        c_l = self.linear_no_bias_ref_pos(
            ...
            jnp.arcsinh(input_feature_dict["ref_charge"].astype(dtype)).reshape(
        ...
        c_l = c_l + self.linear_no_bias_f(
            jnp.concatenate(
                [
                    input_feature_dict[name].astype(dtype).reshape(
        ...
        c_l = c_l * input_feature_dict["ref_mask"].astype(dtype).reshape(*batch_shape, N_atom, 1)
        ...
        q_trunked_list, k_trunked_list, pad_info = rearrange_qk_to_dense_trunk(
            q=[input_feature_dict["ref_pos"].astype(dtype), input_feature_dict["ref_space_uid"]],
            k=[input_feature_dict["ref_pos"].astype(dtype), input_feature_dict["ref_space_uid"]],
            dim_q=[-2, -1],
            dim_k=[-2, -1],
            n_queries=self.n_queries,
            n_keys=self.n_keys,
    ...
    def __call__(self, *, input_feature_dict, s_inputs, s_trunk, z_trunk, pair_mask, x_pred_coords, key, use_embedding=True):
        ...
        def single_structure(x_pred_rep_coords, key):
            ...
            z_pair += self.linear_no_bias_d(
                ((distance_pred[..., None] > self.lower_bins) * (distance_pred[..., None] < self.upper_bins)).astype(z_pair.dtype)
